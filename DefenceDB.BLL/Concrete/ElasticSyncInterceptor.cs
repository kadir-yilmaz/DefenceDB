        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var entry in entries)
        {
            try
            {
                if (entry.State == EntityState.Deleted)
                {
                    var rm = await db.ProductReadModels.FindAsync(entry.Entity.Id);
                    if (rm != null)
                    {
                        db.ProductReadModels.Remove(rm);
                        readModelChanged = true;
                    }
                }
                else
                {
                    var doc = MapToReadModel(entry.Entity);
                    var existing = await db.ProductReadModels.FindAsync(entry.Entity.Id);
                    if (existing != null)
                    {
                        db.Entry(existing).CurrentValues.SetValues(doc);
                    }
                    else
                    {
                        db.ProductReadModels.Add(doc);
                    }
                    readModelChanged = true;
                }
            }
            ...
            {
                logger.LogWarning(ex, "Read Model sync hatası — ProductId: {Id}", entry.Entity.Id);
            }
        }

        if (readModelChanged)
        {
            await db.SaveChangesAsync(); // Yeni scope olduğu için sonsuz döngüye girmez (DefenseProduct entries boş)
            logger.LogDebug("SQL Read Model güncellendi");
        }

        return result;
    }

    private static ProductReadModel MapToReadModel(DefenseProduct product)
    {
